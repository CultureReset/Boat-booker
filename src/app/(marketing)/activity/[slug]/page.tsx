...
import { activities, activityBySlug } from '@/config/taxonomy';
 ...
 *
 * A real, indexable page per activity carrying live inventory plus the
 * destinations where that activity is available — the pattern that makes the
 * long tail of "sunset cruises in Miami" searches land somewhere useful.
 */
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: { canonical: `/activity/${activity.slug}` },
  };
}
...
export default async function ActivityPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const matching = db.charters.filter(
    (charter) => charter.published && !charter.snoozed && charter.activityKeys.includes(activity.key),
  );
  ...
  // Destinations where this activity actually runs, for the internal links.
    ...
    <div className="mx-auto max-w-shell px-4 py-6">
      <nav aria-label="Breadcrumb" className="mb-3">
        <ol className="flex flex-wrap items-center gap-1 text-xs text-ink-muted">
          ...
          <li><Link href="/activity" className="hover:underline">{t('activities', 'title')}</Link></li>
          <li aria-hidden="true">/</li>
          <li className="font-medium text-ink-soft" aria-current="page">{activity.title}</li>
        </ol>
      </nav>

      <SectionHeading title={activity.title} subtitle={activity.blurb} level={1} />
      ...
      {cards.length === 0 ? (
        <EmptyState
          ...
          action={<LinkButton href="/activity">{t('activities', 'title')}</LinkButton>}
        />
      ) : (
        <>
          <h2 className="mb-3 text-base font-bold text-ink">
            {t('activities', 'boatsOffering', { count: matching.length })}
          </h2>
          ...
          {matching.length > cards.length ? (
            <div className="mt-5 text-center">
              <LinkButton href={`/charters/search?activities=${activity.slug}`} variant="outline" size="lg">
